...
npm run test:e2e -w server        # e2e 58 用例 / 4 个套件
```
...
e2e 里有两组**并发回归**用例，它们都是「修复前必红」的：

| 套件           | 盯住的缺陷                                                                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `concurrency`  | 20 个并发建单不重号；同一订单 5 个并发出库只出库一次、库存只扣一次（修复前 5 次全成功、75→50）；并发确认只生效一次；10 个并发订单抢一半库存不超卖 |
| `api-contract` | 11 个列表接口不带参数不再 500；`pageSize` 上限生效；商品/库存列表翻页真的换数据；五个角色的权限矩阵；`@RequirePermissions` 的权限码必须真实存在   |

最后一条是根因防护：曾经 `product:view` 等权限码只写在代码里、没进权限表，
守卫精确匹配永远不命中，导致除超管外**所有角色打开商品/客户/供应商管理都是 403**，
而菜单还显示着。现在这个测试会在缺码时直接失败。
