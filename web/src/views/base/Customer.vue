  ...
  <div class="page">
    <div class="page-card">
      <div class="toolbar">
        ...
        <el-button v-permission="'customer:create'" type="primary" :icon="Plus" @click="openCreate"
          >新增客户</el-button
        >
      </div>
      ...
      <ResponsiveList :items="list" :fields="cardFields" :loading="loading" empty-text="暂无客户">
        <el-table v-loading="loading" :data="list">
          ...
          <el-table-column label="等级" width="90" align="center">
            <template #default="{ row }">
              <el-tag
                v-if="row.level"
                :type="row.level === 'VIP' || row.level === '重点' ? 'warning' : 'info'"
                size="small"
                >{{ row.level }}</el-tag
              >
              <span v-else>-</span>
        ...
        <template #actions="{ row }">
          <el-button v-permission="'customer:update'" link type="primary" size="small" @click="openEdit(row)"
            >编辑</el-button
          >
          <el-popconfirm title="停用该客户？" @confirm="remove(row)">
            <template #reference>
        ...
        <el-row :gutter="12">
          <el-col :span="12">
            <el-form-item label="编码" prop="code"
              ><el-input v-model="form.code" :disabled="!!form.id" placeholder="如 CUS004"
            /></el-form-item>
          </el-col>
          <el-col :span="12">
...
const cardFields: ListField<CustomerItem>[] = [
  { label: '客户名称', prop: 'name', primary: true },
  {
    label: '状态',
