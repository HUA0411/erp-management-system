      ...
      <el-table v-loading="loading" :data="list">
        ...
        <el-table-column label="类型" width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="INVENTORY_TYPE[row.type]?.type || 'info'" size="small">{{
              INVENTORY_TYPE[row.type]?.text || row.type
            }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="变动数量" width="110" align="right">
          <template #default="{ row }">
            <span
              class="num"
              :style="row.quantity < 0 ? 'color:var(--danger-text)' : 'color:var(--success-text)'"
            >
        ...
        <el-table-column label="结余" width="100" align="right">
          <template #default="{ row }"
            ><span class="num">{{ fmtQty(row.balanceAfter) }}</span></template
          >
        </el-table-column>
