  ...
  <div class="page">
    <div class="page-card">
      <div class="toolbar">
        ...
        <el-button v-permission="'sale:order:create'" type="primary" :icon="Plus" @click="openCreate"
          >新增销售订单</el-button
        >
      </div>
          ...
          <el-table-column label="操作" width="250" align="center" fixed="right">
            <template #default="{ row }">
              ...
              <el-button
                ...
                v-permission="'sale:order:update'"
                link
                type="primary"
                size="small"
                @click="openEdit(row)"
              ...
              <el-button
                v-if="row.status === 'draft'"
                v-permission="'sale:order:confirm'"
                link
                type="success"
                size="small"
                @click="confirm(row)"
              ...
              <el-button
                ...
                v-permission="'sale:order:outbound'"
                link
                type="warning"
                size="small"
                @click="outbound(row)"
              ...
              >
                <template #reference>
                  <el-button v-permission="'sale:order:cancel'" link type="danger" size="small"
                    >取消</el-button
                  >
                </template>
              </el-popconfirm>
    ...
    <el-drawer v-model="detailVisible" title="销售订单详情" size="640px">
      <template v-if="detail">
        ...
        <div v-if="detail.status === 'confirmed'" class="detail-actions">
          <el-button v-permission="'sale:order:outbound'" type="warning" @click="outbound(detail)"
            >立即出库</el-button
          >
        </div>
      </template>
...
/**
 * 窄屏卡片的字段定义。
 * 单号是这页的「主键」，放标题；金额/已收/状态一眼要看到；
 * 创建时间放最后（次要信息）。
 */
const cardFields: ListField<SaleOrderItem>[] = [
  ...
  { label: '已收', format: (r) => `¥${fmtMoney(r.paidAmount)}`, numeric: true },
...
async function outbound(row: SaleOrderItem) {
  await ElMessageBox.confirm(`确认对订单 ${row.orderNo} 执行出库？将扣减库存并校验库存充足。`, '销售出库', {
    type: 'warning',
  });
