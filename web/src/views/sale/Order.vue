<template>
  <div class="page">
    <div class="page-card">
      <div class="toolbar">
        <el-input v-model="query.keyword" placeholder="单号 / 客户" clearable style="width: 200px" @keyup.enter="load" @clear="load" />
        ...
        <el-button v-permission="'sale:order:create'" type="primary" :icon="Plus" @click="openCreate">新增销售订单</el-button>
      </div>
      ...
      <el-table :data="list" v-loading="loading">
        ...
        <el-table-column label="操作" width="250" align="center" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'draft'" v-permission="'sale:order:update'" link type="primary" size="small" @click="openEdit(row)">编辑</el-button>
            <el-button v-if="row.status === 'draft'" v-permission="'sale:order:confirm'" link type="success" size="small" @click="confirm(row)">确认</el-button>
            <el-button v-if="row.status === 'confirmed'" v-permission="'sale:order:outbound'" link type="warning" size="small" @click="outbound(row)">出库</el-button>
            <el-popconfirm v-if="['draft', 'confirmed'].includes(row.status)" title="确定取消该订单？" @confirm="cancel(row)">
              <template #reference>
                <el-button v-permission="'sale:order:cancel'" link type="danger" size="small">取消</el-button>
              </template>
            </el-popconfirm>
          </template>
        </el-table-column>
    ...
    <!-- 新增/编辑 -->
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑销售订单' : '新增销售订单'" width="860px" :close-on-click-modal="false" destroy-on-close>
      ...
      <div class="items-head">
        <span class="items-title">商品明细（出库时校验库存）</span>
    ...
    <!-- 详情 -->
    <el-drawer v-model="detailVisible" title="销售订单详情" size="640px">
      <template v-if="detail">
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item label="已收金额">¥{{ fmtMoney(detail.paidAmount) }}</el-descriptions-item>
        ...
        <div v-if="detail.status === 'confirmed'" class="detail-actions">
          <el-button v-permission="'sale:order:outbound'" type="warning" @click="outbound(detail)">立即出库</el-button>
        </div>
      </template>
    </el-drawer>
  </div>
...
async function loadOptions() {
  const [c, p] = await Promise.all([customerApi.options(), productApi.options()]);
  customers.value = c;
...
function openCreate() {
  Object.assign(form, { id: undefined, customerId: undefined, orderDate: today(), remark: '', items: [] });
  addLine();
  dialogVisible.value = true;
}

function openEdit(row: SaleOrderItem) {
  Object.assign(form, {
    id: row.id,
...
async function save() {
  ...
  saving.value = true;
  try {
    const payload = { customerId: form.customerId, orderDate: form.orderDate, remark: form.remark, items: form.items.map((i) => ({ productId: i.productId, quantity: i.quantity, price: i.price })) };
    if (form.id) await saleApi.update(form.id, payload);
    else await saleApi.create(payload);
...
async function confirm(row: SaleOrderItem) {
  await saleApi.confirm(row.id);
  ElMessage.success('订单已确认');
  load();
}

async function cancel(row: SaleOrderItem) {
  await saleApi.cancel(row.id);
  ElMessage.success('订单已取消');
  load();
}

async function outbound(row: SaleOrderItem) {
  await ElMessageBox.confirm(`确认对订单 ${row.orderNo} 执行出库？将扣减库存并校验库存充足。`, '销售出库', { type: 'warning' });
  const res = await saleApi.outbound(row.id);
  ElMessage.success(`出库成功：${res.outboundNo}`);
  detailVisible.value = false;
  load();
}

async function openDetail(id: number) {
  detail.value = await saleApi.detail(id);
