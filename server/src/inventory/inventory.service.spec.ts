...
describe('InventoryService.movement（库存变动核心）', () => {
  ...
  beforeEach(() => {
    service = new InventoryService({ transaction: jest.fn() } as any, {} as any, {} as any, {} as any);
  });
  ...
  it('库存不足时抛业务异常且不写入', async () => {
    const manager = makeManager([{ id: 9, quantity: '10' }]);
    await expect(service.movement(manager, { ...baseOpts, delta: -20 })).rejects.toThrow(BusinessException);
    // 只允许 SELECT 查询执行，UPDATE/INSERT 不应发生
    const updateCalls = (manager.query as jest.Mock).mock.calls.filter(
      ([sql]) => String(sql).startsWith('UPDATE') || String(sql).startsWith('INSERT'),
    );
  ...
  it('无库存行且为负数变动时拒绝', async () => {
    ...
    await expect(service.movement(manager, { ...baseOpts, delta: -1 })).rejects.toThrow(BusinessException);
  });
  ...
  it('无库存行且为正数变动时插入新行', async () => {
    ...
    expect(manager.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO inventory'), [1, 1, 5]);
  });
